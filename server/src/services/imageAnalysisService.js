...
async function analyzeImage(imageBuffer) {
  try {
    ...
    const tempFilePath = path.join(__dirname, `../../temp/image_${timestamp}.jpg`);
    ...
    const ossUrl = await uploadToOSS(tempFilePath, ossKey);
    ...
    return {
      ...
      result: {
        ...
        description: behaviorDesc || BEHAVIOR_TYPES[behaviorType]?.description || '未知行为'
      }
    };
  } catch (error) {
        ...
        messages: [
          {
            role: 'user',
            content: [
              { type: 'text', text: '请分析这张图片中的人物行为，并从以下7种行为中选择一种最匹配的：1.认真专注工作 2.吃东西 3.用杯子喝水 4.喝饮料 5.玩手机 6.睡觉 7.其他。请给出你的选择（只需回复数字和行为描述即可）。' },
...
function extractBehaviorType(analysisText) {
  // 尝试匹配行为类型数字和描述（1-7）
  const pattern = /(\d+)\s*[.、:]?\s*(认真专注工作|吃东西|用杯子喝水|喝饮料|玩手机|睡觉|其他)/;
  ...
  for (const { pattern, type } of patterns) {
    if (pattern.test(analysisText)) {
      return { behaviorType: type, behaviorDesc: pattern.toString().replace(/\\/g, '').replace(/\//g, '') };
    }
  }
