...
const AttentionCard = ({ className }: AttentionCardProps) => {
  ...
  useEffect(() => {
    if (detectionData && detectionData.attention) {
      // 更新热力图和时间槽数据
      const heatmapData = detectionData.attention.heatmap || [];
      
      // 处理可能的数据格式差异
      let formattedHeatmap: HeatmapPoint[] = [];
      if (heatmapData.length > 0) {
        if (Array.isArray(heatmapData[0])) {
          // 数组格式 [x, y, intensity]
          formattedHeatmap = (heatmapData as [number, number, number][]).map(point => ({
            x: point[0],
            y: point[1],
            intensity: point[2]
          }));
        } else if (typeof heatmapData[0] === 'object') {
          // 已经是对象格式
          formattedHeatmap = heatmapData as HeatmapPoint[];
        }
      }
      ...
      // 更新当前注意力值
      const newAttention = detectionData.attention.current_attention || 0;
