...
const fetchRealData = async () => {
  ...
  try {
    ...
    // 确保注意力数据存在并格式正确
    if (!formattedResult.attention) {
      // 如果attention不在顶层，尝试从result中获取
      if (formattedResult.result && formattedResult.result.attentionData) {
        formattedResult.attention = formattedResult.result.attentionData;
      } else {
        // 提供默认值
        formattedResult.attention = {
          heatmap: [],
          time_slot: {},
          current_attention: 0
        };
    ...
    // 确保heatmap格式正确
    if (formattedResult.attention && Array.isArray(formattedResult.attention.heatmap)) {
      const formattedHeatmap = formattedResult.attention.heatmap.map((point: any) => {
        if (Array.isArray(point) && point.length === 3) {
          return point as [number, number, number];
        }
        // 如果是对象格式，转换为数组格式
        if (typeof point === 'object' && 'x' in point && 'y' in point && 'intensity' in point) {
          return [point.x, point.y, point.intensity] as [number, number, number];
        }
        return [0, 0, 0] as [number, number, number];
      });
      formattedResult.attention.heatmap = formattedHeatmap;
    }
    
    globalState.data = formattedResult;
